...
<dict>
    ...
    <!-- Sparkle auto-update.
         - SUFeedURL: appcast XML location. PLACEHOLDER below; replace once the
           GitHub repo / Releases page is set up.
         - SUPublicEDKey: base64 of the EdDSA public key. Generated by Sparkle's
           generate_keys tool (private key goes into release.sh, public key here).
           Update both at once when releasing.
         - SUEnableAutomaticChecks: true → quiet daily background check. -->
    <key>SUFeedURL</key>
    <string>https://example.invalid/ccbar/appcast.xml</string>
    <key>SUEnableAutomaticChecks</key>
    <true/>
    <!-- Empty until generate_keys is run. Sparkle will refuse to install updates
         without this, so we'll fill it in alongside SUFeedURL when the GitHub
         pipeline is ready. -->
